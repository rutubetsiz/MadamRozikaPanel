...
$(document).ready(function () {
	...
	if ($('.plupload').length > 0) {
		$(".plupload").each(function () {
			...
			$el.pluploadQueue({
				...
				url                : '/static/js/plupload/upload.php',
				max_file_size      : '10mb',
				chunk_size         : '1mb',
				unique_names       : true,
				resize             : {
		...
		$('.dataTable').each(function () {
			...
			if ($el.hasClass('dataTable-tools')) {
				...
				dataTable_options.tableTools = {
				    "sSwfPath": "/static/js/plugins/datatables/extensions/copy_csv_xls_pdf.swf"
				};
			}
	...
	if ($('.file-manager').length > 0) {
		$('.file-manager').elfinder({
		    url: '/static/js/plugins/elfinder/php/connector.php'
		});
	}
