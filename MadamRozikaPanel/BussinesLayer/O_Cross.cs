...
namespace MadamRozikaPanel.BussinesLayer
{
    ...
    {
        MadamRozikaEntities _db=new MadamRozikaEntities();
        //public CheckBoxList ReturnDigerKategoriler(ref CheckBoxList cbl, int ContentId)
        //{
        //    var result = _db.News.Include(x=>x.CategoryNewsRelations).Include(xt=>xt.CategoryNewsRelations.Select(t=>t.Category)).Where(n=>n.NewsId==);
        //    //foreach (var item in _db.News.Include(x=> x.))
        //    //{
